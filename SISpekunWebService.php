<?php
	...
	function getPeminjaman($idPeminjam,$tipePeminjam)
	{
		...
		$bulan = date("m");
		$tahun = date("Y");
		die("SELECT MAHASISWA.Nama as NamaPeminjam, MAHASISWA.NPM as Identitas, PEMINJAMAN.No_Spekun FROM MAHASISWA,PEMINJAMAN WHERE MAHASISWA.NPM = PEMINJAMAN.NPM_Mahasiswa AND Tanggal = $tanggal AND Bulan = $bulan AND Tahun = $tahun AND (Status = NULL OR Status = 0)  AND MAHASISWA.NPM=$idPeminjam");
		if ($tipePeminjam == "Mahasiswa")
		{
